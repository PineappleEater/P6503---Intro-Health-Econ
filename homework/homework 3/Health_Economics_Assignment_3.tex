...
\begin{tikzpicture}[scale=0.95]
    ...
    % Market Eq: MPB = MSC => 5-Q = 1+Q => Q=2, P=3
    \draw[dashed] (2,0) node[below] {$Q_0$} -- (2,3);
    \draw[dashed] (0,3) node[left] {\footnotesize $P_0$} -- (2,3);
    \filldraw[black] (2,3) circle (2pt);
    \node[font=\footnotesize, anchor=south west] at (2.05,3.05) {Market Eq};
    
    % Social Eq: MSB = MSC => 4-Q = 1+Q => Q=1.5, P=2.5
    \draw[dashed] (1.5,0) node[below] {$Q_1$} -- (1.5,2.5);
    \filldraw[black] (1.5,2.5) circle (2pt);
    \node[font=\footnotesize, anchor=north east] at (1.45,2.45) {Social Eq};

    % MEC annotation
    \draw[<->, thick, magenta] (4.3, 0.7) -- (4.3, 1.7);
    \node[magenta, font=\footnotesize, anchor=west] at (4.4, 1.2) {MEC};
\end{tikzpicture}
\end{center}
...
\textit{Correction Note: The given Total Cost function $TC = 500 + 10Q + 20Q^2$ is mathematically inconsistent with its given Marginal Cost derivative ($MC = 10 + 2Q$), which strictly implies the intended function was $TC = 500 + 10Q + Q^2$. Using $20Q^2$ would result in massively negative profits and an economic contradiction. Therefore, the mathematically consistent function $TC = 500 + 10Q + Q^2$ is applied for all profit calculations below.}
